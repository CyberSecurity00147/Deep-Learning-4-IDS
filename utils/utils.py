...
def readucr(filename):
    ...
    #this function is customized to read arff files
    data, _ =  arff.loadarff(filename)

    x_data = []
    y_data = []
    for x in data:
        x_data.append(list(x)[:len(x)-2])
        y_data.append(list(x)[-1])
    return np.asarray(x_data), np.asarray(y_data)
    
    


def readucr_txt(filename):
...
def read_dataset(root_dir,archive_name,dataset_name):
    ...
    if archive_name == 'mts_archive':
        ...
        x_test = np.load(file_name + 'x_test.npy',allow_pickle=True)
        y_test = np.load(file_name + 'y_test.npy',allow_pickle=True)
        ...
        x_train, y_train = readucr(file_name+'_TRAIN.arff')
        x_test, y_test = readucr(file_name+'_TEST.arff')
        datasets_dict[dataset_name] = (x_train.copy(),y_train.copy(),x_test.copy(),
            y_test.copy())
...
def transform_mts_to_ucr_format():
    mts_root_dir = './tempoarch/mts_archive/'
...
def calculate_metrics(y_true, y_pred,duration,y_true_val=None,y_pred_val=None,tresh=None):
    if tresh==None:
        y_pred = encode_pred(y_pred)
        print("metrics calculation for the calssification")
    else:
        print("metrics for reconstruction ")
        y_pred = encode_pred_rec(y_pred,tresh)
...
def encode_pred(y_pred):
    ...
    def encd(x):
        if x>0.5:
            ...
            return 0
    return np.array([encd(x) for x in y_pred])

def encode_pred_rec(y_pred,tresh):
...
def get_info_run():
    return {'archive_names': ARCHIVE_NAMES,
            'mts_dataset_names': MTS_DATASET_NAMES,
...
def save_logs(output_directory, hist, y_pred,y_true,duration,
               lr=True,y_true_val=None,y_pred_val=None, tresh=None):
    ...
    if hist!=None:
        ...
        # plot losses
        plot_epochs_metric(hist, output_directory+'epochs_loss.png')
    np.save(output_directory+'y_true',y_true)
    np.save(output_directory+'y_pred',y_pred)
    df_metrics = calculate_metrics(y_true,y_pred, duration,y_true_val,y_pred_val,tresh)
    df_metrics.to_csv(output_directory+'df_metrics.csv', index=False)
    #return df_metrics
